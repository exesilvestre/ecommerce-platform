...
class OrderService:
    ...
    async def create_order(
        ...
        *,
        ...
        await self.customer_service.ensure_exists(
            db=db, customer_id=payload.customer_id
        )
        products_by_id = await self.product_service.get_products_by_ids(
        ...
        stripe_idempotency_key = idempotency_key or f"order-{order_id}"

        # Release the request-scoped connection before external payment.
        await db.close()
        ...
        try:
            ...
            try:
                ...
                async with session_factory() as db_after_payment:
            ...
            if idempotency_key:
                async with session_factory() as db_cleanup:
                    await self.idempotency_service.abandon(
                        db=db_cleanup, key=idempotency_key
                    )
            raise
