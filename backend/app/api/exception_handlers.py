...
def register_exception_handlers(app):
    ...
    async def customer_not_found(_: Request, exc: CustomerNotFoundError):
        return JSONResponse(
            status_code=404, content={"detail": str(exc) or ERR_CUSTOMER_NOT_FOUND}
        )
    ...
    async def no_warehouse_available(_: Request, exc: NoWarehouseAvailableError):
        return JSONResponse(
            status_code=422, content={"detail": str(exc) or ERR_NO_WAREHOUSE}
        )

    @app.exception_handler(InsufficientStockError)
    async def insufficient_stock(_: Request, exc: InsufficientStockError):
        return JSONResponse(
            status_code=422, content={"detail": str(exc) or ERR_INSUFFICIENT_STOCK}
        )

    @app.exception_handler(PaymentFailedError)
    async def payment_failed(_: Request, exc: PaymentFailedError):
        return JSONResponse(
            status_code=402, content={"detail": str(exc) or ERR_PAYMENT_FAILED}
        )
